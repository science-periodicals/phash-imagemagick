...
module.exports = function(s, callback) {
  ...
  var rgx1 = /^( *)(.+?): (.*)$/; // key: val
  var rgx2 = /^( *)(.+?):$/; // key: begin nested object
  ...
  child.on('close', function(code) {
    ...
    if (
      ~['PixelsPerInch', 'PixelsPerCentimeter'].indexOf(data.units) &&
      data.ppi
    ) {
    ...
    data.pHash = data.pHash.reduce(function(hash, x) {
      var value = ('0000' + (100 * x).toFixed()).slice(-4);
      if (x < 0) {
        value = '-' + value.replace('-', '');
      }
