...
var absPaths = fs
  ...
  .filter(function(p) {
    return path.extname(p) === '.png' || path.extname(p) === '.jpg';
  })
  .map(function(p) {
    return path.join(root, 'fixtures', p);
  });

var color = absPaths.filter(function(p) {
  return path.extname(p) === '.png' && path.basename(p).indexOf('_gray') === -1;
});
var colorLow = absPaths.filter(function(p) {
  return path.extname(p) === '.jpg' && path.basename(p).indexOf('_gray') === -1;
});
var gray = absPaths.filter(function(p) {
  return path.extname(p) === '.png' && path.basename(p).indexOf('_gray') !== -1;
});
var grayLow = absPaths.filter(function(p) {
  return path.extname(p) === '.jpg' && path.basename(p).indexOf('_gray') !== -1;
});
  ...
  describe('pHash', function() {
    it('should get perceptual hash values', function(done) {
      pHash.get(color[0], function(err, data) {
        ...
        assert.equal(
          data.pHash,
          '006100520470037004230398044704220885087406860629092808360061-030043101600421-0100430004008700058064801210870010600640061047804480499042904930441102708880745066609930896'
        );
        done();
      });
    });
  ...
  describe('pHash comparison', function() {
    ...
    before(function(done) {
      async.parallel(
        {
          color: function(cb) {
            async.mapSeries(color, pHash.get, cb);
          },
          colorLow: function(cb) {
            async.mapSeries(colorLow, pHash.get, cb);
          },
          gray: function(cb) {
            async.mapSeries(gray, pHash.get, cb);
          },
          grayLow: function(cb) {
            async.mapSeries(grayLow, pHash.get, cb);
          }
        },
        function(err, res) {
          hashes = res;
